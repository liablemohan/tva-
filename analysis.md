...
**Scope:** `index.html` (home page), its logic in `hero-animation-controller.js`, `forge-canvas.js`, `image-slot.js`.
...
- Email input, `index.html:142`: `width: 260px` — doesn't shrink with its flex-wrap row on narrow screens.
...
`index.html:33-35`: button is `width: 30px`, two 1.6px bars with a 6px gap — visible glyph height ~14px, well under the 44×44px touch-target guideline (WCAG 2.5.5 / Apple & Material HIG).
...
`#667085` on the page ground `#F5F3EE` measures **4.49:1** — just under the 4.5:1 AA threshold for normal text. It's used for the 12–13px monospace eyebrow/meta labels throughout the page (`index.html:43,47,178,190`), which is exactly where a marginal ratio matters most (small text has no size-based exemption).
